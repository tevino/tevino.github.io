...
.PHONY: serve
serve: ## Start dev server without a TTY (detached) — use in non-TTY shells where `make dev` fails
	@echo "Starting Jekyll dev server without TTY..."
	docker run --rm -d \
		--name $(CONTAINER_NAME) \
		-p $(PORT):4000 \
		-v "$(PWD)":/srv/jekyll \
...
.PHONY: build
build: ## Build site for production (non-TTY)
